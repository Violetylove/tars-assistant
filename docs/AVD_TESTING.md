...
- 桌面图标错位问题（新发现）：模型在桌面点 `[3] text"Gmail"`（小组件文字）后实际进入 Google 日历而非 Gmail；桌面轮模型输入里 `[9] text"TARS Assistant / 预测的"` 与真实可点图标错位。根因疑似桌面图标的文字标签与可点击目标（坐标对应图标）不一致，导致模型按文字决策、点击落点却命中其他图标。待进一步排查桌面节点文字/可点目标映射。
