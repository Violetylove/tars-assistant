...
观察新鲜度规则：动作前保存 UI XML、前台包名和无障碍事件序号；无障碍服务须订阅窗口、内容、焦点、文本和选中事件。动作后以前台包名变化为主信号、XML 差异为辅助信号，焦点/内容/窗口事件作为 UI 正在刷新的信号。若事件已到达但 XML 尚未更新，只允许一次短暂延迟重采样；仍受观察超时和最大轮数限制，超时即停止。
