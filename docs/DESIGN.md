...
- [x] 会话历史由原生 App 回传：决策轮数上限 12、历史最多七轮且只含已执行的合法 action；Agent 保持无状态，重启不会残留任务上下文
- [ ] 云端模型服务商的超时、额度与失败重试策略实测标定：正常链路已完成一次无 UI、无动作
  健康探针（HTTP 200，约 2.84 秒）；设备内一次 `ConnectionError` 已按配置重试 2 次后安全返回 502，
  未执行任何动作。配额、限流和超时的真实失败样本仍待在不干扰服务的窗口采集
- [x] 设备内云端代理链：清除遗留 ADB `tcp:10000` 转发和 Termux 代理环境变量后，Agent 可直接访问国内云端模型；Android↔Agent 仍仅使用 `127.0.0.1:8080`
- [ ] Gmail 通用表单链：启动并聚焦收件人字段已验证；仍需确认云端在观察轮继续生成 `type`，再复验通用输入执行，不增加 Gmail 专用状态机

## 11. 运行时可配置参数登记

所有影响运行时行为、且计划开放或已开放给用户配置的参数，集中登记在
[`docs/RUNTIME_CONFIG.md`](./RUNTIME_CONFIG.md)：按「A 类计划用户可配置 / B 类安全白名单（不开放）/
C 类已配置化（私有配置承载）」分类，标注默认值、代码归属与校验边界。

- A 类（如观察轮数、loopback 地址/端口、模型请求超时）是二期用户设置界面的配置项候选，开放时须过
  范围与 loopback 校验并支持恢复安全默认值。
- B 类（动作白名单、可启动包名、敏感标签、协议边界）为安全边界，严禁放宽，改一处必须同步两端。
- C 类（云端 `config/cloud.yaml`）由私有文件承载，不经 Android 设置页展示或编辑。
- 新增或调整任何运行时参数前，先在本登记表登记，再实现代码（约定见 §2 协议先行）。
