...
- [x] 建立参数登记基础：`docs/RUNTIME_CONFIG.md` 已登记全部运行时可配置参数，按「A 类计划用户可配置 / B 类安全白名单（不开放）/ C 类已配置化」分类，标注默认值、代码归属与校验边界。
- [ ] 提供面向用户的本地设置界面，用于配置受限运行参数，例如单任务最大观察轮数、Agent
  loopback 地址和超时；所有值均须做范围与 loopback 校验（以 `RUNTIME_CONFIG.md` A 类清单为准）。
...
| 2026-08-22 | 多轮容量再调整：用户实测发现 Gmail 填入收件人后会动态弹出「建议」匹配组件（通讯录好友 Winter Yuan），突发组件耗尽观察预算导致模型未能继续填主题/正文/发送；确认根因非采集缺失而是轮数不足，将决策轮数上限从 8 提高到 12（`MainActivity.MAX_OBSERVATION_ROUNDS`），history 上限与单轮动作上限不变。 |
